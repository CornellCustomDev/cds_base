// Cornell Design System UI (last update: 11/10/25)
...
jQuery(document).ready(function($) {	
	...
	// 06. Animated Anchor Link ------------------------------------------------
	$('.anchor-link').each(function() {
		var target = $(this).attr('data-target') || $(this).attr('href') || '#'; // An empty anchor is the fallback target if not defined
		// @todo: this fallback should be improved, to ensure that links to anchors on other pages do not cause the link to simply do nothing
		
		$(this).click(function(e) {
			e.preventDefault();
			//e.stopPropagation();
			var target_offset = Math.floor($(target).offset().top);
			//console.log($(window).scrollTop() + ' vs. ' + target_offset);
			var duration = Math.floor(Math.abs((target_offset - $(window).scrollTop()) / 2)); // scale the animation duration based on scrolling distance
			duration = Math.min(Math.max(duration,400),1000); // duration is capped between 400ms and 1000ms
			//$(target).focus(); // @todo: do we need/want to ensure the target is focusable, by adding a tabindex of -1?
